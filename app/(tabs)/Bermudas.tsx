...
export default function Camiseta() {
  ...
  const [newNameBermuda, setNewNameBermuda] = useState('');
  const [newPrecoBermuda, setNewPrecoBermuda] = useState('');
  const [newImageBermuda, setNewImageBermuda] = useState('');
  ...
  const add = async () => {
    if (newNameBermuda === "" && newNameBermuda === "" && newPrecoBermuda === "") {
        Alert.alert("Por favor, insira um produto!");
        return;
    }
    await addDoc(collection(FIREBASE_DB,"bermuda"), { name: newNameBermuda, image: newImageBermuda, preco: newPrecoBermuda });
    setNewNameBermuda('');
    setNewPrecoBermuda('');
    setNewImageBermuda('');
  };
  ...
  const deleteItem = async (id: string) => {
    await deleteDoc(doc(FIREBASE_DB, "bermuda", id));
  };
  ...
  const update = async (id: string) => {
    ...
    const userRef = doc(FIREBASE_DB, "bermuda", id);

    await updateDoc(userRef, {
      name: newNameBermuda,
      image: newImageBermuda,
      preco: newPrecoBermuda
    });
      ...
      <SafeAreaView>
        <View>
          <TextInput
            ...
            value={newNameBermuda}
            onChangeText={setNewNameBermuda}
          />
          <TextInput
            style={styles.input}
            placeholder="Link da imagem do produto"
            value={newImageBermuda}
            onChangeText={setNewImageBermuda}
          />
          <TextInput
            style={styles.input}
            placeholder="Preço"
            value={newPrecoBermuda}
            onChangeText={setNewPrecoBermuda}
          />
